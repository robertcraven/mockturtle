...
Doing a `mockturtle.py -h` from the command line will show command-line
usage:
...
From within the interpreter, since it subclasses `cmd.Cmd`, you can
enter `?` to receive help, or `help X` to receive help on a specific
command `X`, e.g., `help move`.

## Platform

The program is written in python (> 3.6) and has been tested on
Ubuntu Linux 18.04.  (It has not been written to run on Macs or Windows.
Indeed, for the former the issue mentioned at https://bit.ly/2GbGBsI has
been observed to arise.)
...
Three modes of running are supported.
 ...
 - The first text-based method is by running the TurtleShell after
   importing this file as a module.  See [Example 2] and
   [Example 4].
...
be typed into a command shell ([Example 1] and [Example 2], below), or
can be loaded from a file whose name is passed as input ([Example 3]
and [Example 4]).

Otherwise, commands are passed using functions within the `Turtle`
objects, as in [Example 5].
...
of a line drawn.   (See [Program structure] for more details.)
...
[Example 1].  We read a program from a file (using `-p`),
...
This is similar to [Example 2], except we read from a file.
...
Using `tkinter` (as `tk`) for graphics, we open a root window, and for
the main application subclass `tk.Frame` (as `TurtleApp`).  This packs
a canvas, which can be dragged to show what stray turtles are up to.
...
Animation is simulated by drawing lines in segments and delaying between
each segment.  The number of segments is controlled indirectly by the
...
though `CTRL-D` does.)  If the application is running a program passed
from file, then either close the window or `CTRL-C` in the terminal
should work.
